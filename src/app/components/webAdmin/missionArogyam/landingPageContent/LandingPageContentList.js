...
export default class LandingPageContentList extends React.Component{
    ...
    loadData =(page =1)=>{
        ...
        getAPI(MISSION_LANDING_PAGE_CONTENT ,successFn, errorFn, apiParams);
    };
    ...
    deleteObject(record) {
        ...
        patchAPI(interpolate(MISSION_SINGLE_LANDING_PAGE_CONTENT, [record.id]), reqData, successFn, errorFn)
    }
...
<div><Switch>
            ...
            <Route
              ...
              render={(route) => <AddLandingPageContent {...this.state} {...route} loadData={this.loadData} />}
            />
            <Card title="Landing Page Content" extra={<Link to="/mission/landingpagecontent/add"> <Button type="primary"><Icon type="plus" /> Add</Button></Link>}>
                <List
...
<List.Item
  ...
  actions={[<Link to={`/mission/landingpagecontent/edit/${item.id}`}>Edit</Link>,
                                                         <Popconfirm
