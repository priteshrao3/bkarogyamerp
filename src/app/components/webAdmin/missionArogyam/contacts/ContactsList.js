...
import {MISSION_BLOG_CONTACTUS, MISSION_SINGLE_CONTACT} from "../../../../constants/api";
...
export default class ContactsList extends React.Component {
    ...
    loadData =(page = 1)=> {
        ...
        getAPI(MISSION_BLOG_CONTACTUS, successFn, errorFn, apiParams);
    }
    ...
    deleteObject(record) {
        ...
        patchAPI(interpolate(MISSION_SINGLE_CONTACT, [record.id]), reqData, successFn, errorFn)
    }
        ...
        const columns = [{
        ...
        },
            {
                ...
                render: (item) => {
                    return (
<div>
                        <Link to={`/mission/contact/edit/${  item.id}`}>Edit</Link>
...
<div><Switch>
            <Route
              exact
              path='/mission/contact/add'
              render={(route) => <AddContacts {...this.state} {...route} loadData={this.loadData} />}
            />
            <Route
              exact
              path='/mission/contact/edit/:id'
              render={(route) => <AddContacts {...this.state} {...route} loadData={this.loadData} />}
            />
            <Card
              title="Contacts"
              extra={<Link to="/mission/contact/add"> <Button type="primary"><Icon type="plus" /> Add</Button></Link>}
            >
