...
import {MISSION_BLOG_EVENTS, MISSION_BLOG_POST,MISSION_SINGLE_EVENTS} from "../../../../constants/api";
...
export default class DiseaseList extends React.Component {
    ...
    loadData =(page =1 )=> {
        ...
        getAPI(MISSION_BLOG_EVENTS, successFn, errorFn, apiParams);
    };
    ...
    deleteObject(record) {
        ...
        patchAPI(interpolate(MISSION_SINGLE_EVENTS, [record.id]), reqData, successFn, errorFn)
    }
    ...
    render() {
        ...
        const columns = [{
            ...
            render: (item) => {
                return (
<div>
                    <Link to={`/mission/event/edit/${  item.id}`}>Edit</Link>
...
<div><Switch>
            <Route
              exact
              path='/mission/event/add'
              render={(route) => <AddEvent {...this.state} loadData={this.loadData} {...route} />}
            />
            <Route
              exact
              path='/mission/event/edit/:id'
              render={(route) => <AddEvent {...this.state} loadData={this.loadData} {...route} />}
            />
            <Card
              title="Events"
              extra={<Link to="/mission/event/add"><Button type="primary"><Icon type="plus" /> Add</Button></Link>}
            >
