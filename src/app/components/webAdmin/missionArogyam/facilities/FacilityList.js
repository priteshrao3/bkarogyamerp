...
export default class FacilityList extends React.Component {
    ...
    loadData =(page =1) => {
        ...
        getAPI(MISSION_BLOG_FACILITY, successFn, errorFn, apiParams);
    };
    ...
    deleteObject(record) {
        ...
        patchAPI(interpolate(MISSION_SINGLE_FACILITY, [record.id]), reqData, successFn, errorFn)
    }
    ...
    render() {
        ...
        const columns = [{
            ...
            render: (item) => {
                return (
<div>
                    <Link to={`/mission/facilities/edit/${  item.id}`}>Edit</Link>
...
<div><Switch>
            ...
            <Card
              title="Facilities"
              extra={(
<Link to="/mission/facilities/add"> <Button type="primary"><Icon
  type="plus"
/> Add
                                </Button>
</Link>
