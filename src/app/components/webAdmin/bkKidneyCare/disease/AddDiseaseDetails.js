...
export default class AddDiseaseDetails extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            disease:[],
    ...
    render() {
        ...
        const fields = [{
            label: 'Disease ',
            key: 'disease',
            initialValue: this.state.editBlogData ? this.state.editBlogData.disease : null,
            type: SELECT_FIELD,
            required: true,
            options: this.state.disease.map(item => {
        ...
        if (this.state.editBlogData) {
            editformProp = {
                ...
                action: interpolate(SINGLE_DISEASE,[this.state.editBlogData.id]),
                method: 'put',
