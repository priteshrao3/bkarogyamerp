...
export default class MonthlyInventory extends React.Component {
    ...
    render() {
        ...
        for (j = 0; j < currentData.length; j++) {
            if(currentData[j].date in itemDate){
                itemDate[currentData[j].date][currentData[j].type_of_consumption] = currentData[j].consume;
            }else{
                itemDate[currentData[j].date] = {"SERVICES": 0,"SALES": 0,"DAMAGED": 0,"RETURNED": 0,"ADJUSTMENT": 0};
                itemDate[currentData[j].date][currentData[j].type_of_consumption] = currentData[j].consume;
            }
        }
        ...
        for(const key in itemDate){
            ...
            const total = itemDate[key].SERVICES + itemDate[key].SALES + itemDate[key].DAMAGED + itemDate[key].RETURNED + itemDate[key].ADJUSTMENT;
