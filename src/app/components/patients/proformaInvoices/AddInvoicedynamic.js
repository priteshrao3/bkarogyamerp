...
import {
    ...
    PROCEDURE_CATEGORY, PROFORMA_INVOICE, PROFORMA_INVOICE_DETAIL,
    SEARCH_THROUGH_QR,
    SINGLE_INVOICE_API,
    TAXES,
...
class Addinvoicedynamic extends React.Component {
    ...
    loadEditInvoiceData = () => {
        ...
        getAPI(interpolate(PROFORMA_INVOICE_DETAIL, [that.props.editId]), successFn, errorFn);
    };
    ...
    handleSubmit = (goBack) => {
        ...
        that.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                if (that.props.editId) {
                    putAPI(interpolate(PROFORMA_INVOICE_DETAIL, [that.props.editId]), reqData, successFn, errorFn);
                } else {
                    postAPI(PROFORMA_INVOICE, reqData, successFn, errorFn);
                }
            ...
            <div>
                <Spin spinning={this.state.saveLoading} tip="Saving Invoice...">
                    <Spin spinning={this.state.promoCodeCheckLoading} tip="Checking Promo Code and Discounts...">
                        <Card
                          title={this.props.editId ? `Edit Proforma Invoice ` : "Add Proforma Invoice"}
                          extra={(
                                <Search
