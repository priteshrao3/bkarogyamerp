...
export default class CustomizedTable extends React.Component {
    ...
    pdfExport() {
        ...
        that.state.dataSource.forEach(function(dataRow, index) {
            ...
            that.state.columns.forEach(function(column) {
                if (column.export) {
                    dataObjectToPush[column.title] = column.export(_get(dataRow, column.dataIndex), dataRow, index);
                } else {
                    dataObjectToPush[column.title] = _get(dataRow, column.dataIndex);
                }
            });
    ...
    getColumnSearchProps = dataIndex => ({
        ...
        onFilter: (value, record) =>
            _get(record,dataIndex) != null ? _get(record,dataIndex)
                .toString()
                .toLowerCase()
